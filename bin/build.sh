...
# West Build (left)
time west build -s zmk/app -d build/left -b adv360_left -- -DZMK_CONFIG="${PWD}/config" -Wno-dev
# # Adv360 Left Kconfig file
# cat build/left/zephyr/.config | grep -v "^#" | grep -v "^$"
# West Build (right)
time west build -s zmk/app -d build/right -b adv360_right -- -DZMK_CONFIG="${PWD}/config" -Wno-dev
